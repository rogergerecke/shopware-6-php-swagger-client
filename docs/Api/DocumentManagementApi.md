# OpenAPI\Client\DocumentManagementApi
...
```php
createDocument($order_id, $document_type_name, $file_type, $inline_object): \OpenAPI\Client\Model\InlineResponse200
```
...
$apiInstance = new OpenAPI\Client\Api\DocumentManagementApi(
...
try {
    $result = $apiInstance->createDocument($order_id, $document_type_name, $file_type, $inline_object);
    print_r($result);
} catch (Exception $e) {
...
```php
numberRangeReserve($type, $saleschannel, $preview): \OpenAPI\Client\Model\InlineResponse20010
```
 ...
 **saleschannel** | **string**| Sales channel for the number range. Number ranges can be defined per sales channel, so you can pass a sales channel ID here. | [optional]
...
```php
uploadToDocument($document_id, $file_name, $extension, $body): \OpenAPI\Client\Model\InlineResponse200
```
